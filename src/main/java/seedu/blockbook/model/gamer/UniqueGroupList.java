 ...
 * Group uniqueness is case-insensitive.
 */
public class UniqueGroupList implements Iterable<Group> {
    ...
    public boolean contains(Group toCheck) {
        ...
        return internalList.stream().anyMatch(group -> isSameGroup(group, toCheck));
    }
    ...
    private boolean isSameGroup(Group first, Group second) {
        return first.toString().equalsIgnoreCase(second.toString());
    }

    /**
     * Returns true if {@code groups} contains only unique groups (case-insensitive).
     */
    private boolean groupsAreUnique(List<Group> groups) {
        for (int i = 0; i < groups.size() - 1; i++) {
            for (int j = i + 1; j < groups.size(); j++) {
                if (isSameGroup(groups.get(i), groups.get(j))) {
                    return false;
                }
            }
        }
